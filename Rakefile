...
task :bundle_cookbook, :cookbook do |t, args|
 ...
 system("tar", "-C", temp_dir, "-cvzf",
        File.join(tarball_dir, tarball_name), "./#{args.cookbook}")

 FileUtils.rm_rf temp_dir
end

# load rake tasks
Dir[File.expand_path(File.join(File.dirname(__FILE__),'tasks','**','*.rake'))].each {|f| load f}
